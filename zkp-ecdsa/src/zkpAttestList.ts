...
import { tomEdwards256, secp256k1 } from './curves/instances.js'

import { Group } from './curves/group.js'
import { cmpArray } from './util.js'
import { WeierstrassGroup } from './curves/weier.js'
...
export async function keyToInt(publicKey: CryptoKey, curve: WeierstrassGroup = secp256k1): Promise<bigint> {
    const pkBytes = new Uint8Array(await crypto.subtle.exportKey('raw', publicKey)),
        pkPoint = curve.deserializePoint(pkBytes),
...
export async function proveSignatureList(
    ...
    const len = sigBytes.length,
        ...
        paramsSigExp = new PedersenParams(curve, R, params.NistGroup.h),
